...
# Upstream's name for the flat cap this fork replaced with the tiered
# `_char_cap_for`. Bound to the slice cap because that is what upstream's
# tests import it as — the size past which a document is sliced.
_FILE_CHAR_CAP = _TEXT_SLICE_CHARS
...
def _estimate_file_tokens(unit: "Path | FileSlice") -> int:
    ...
    if path.suffix.lower() == ".pdf":
        try:
            content = _pdf_text_for_estimate(path)[:cap]
        except Exception:
            return 0
    ...
    if _TOKENIZER is None:
        return int((len(content) + _PER_FILE_OVERHEAD_CHARS) / _FALLBACK_CHARS_PER_TOKEN)
    return len(_TOKENIZER.encode(content, disallowed_special=())) + (_PER_FILE_OVERHEAD_CHARS // _CHARS_PER_TOKEN)
...
def _context_lost_result(chunk, model: str | None, *, is_timeout: bool = False) -> dict:
    ...
    A TIMEOUT keeps the older ``"stop"``: that path is upstream's, the file is
    left unstamped and retried next run, and the incomplete-build guard (#3105)
    already keeps such a run from overwriting a larger graph — so it does not
    depend on this label the way a refusal did.
    """
    return {
        "nodes": [], "edges": [], "hyperedges": [],
        "input_tokens": 0, "output_tokens": 0,
        "model": model,
        "finish_reason": "stop" if is_timeout else "context_exceeded",
    ...
    *,
    ...
    try:
        ...
        if smaller_out < _MIN_OUTPUT_TOKENS:
            smaller_out = None
        # Only a CONTEXT refusal is on `input + requested output`, so only it can
        # be answered by asking for less output. A timeout means the request was
        # accepted and took too long; shrinking the budget there neither helps
        # nor converges, and it would relabel a give-up as `context_exceeded`.
        if is_timeout:
            smaller_out = None
        ...
        if len(chunk) <= 1:
            ...
            return _context_lost_result(chunk, model, is_timeout=is_timeout)
        if _depth >= max_depth:
        ...
        print(
            ...
            f"{_depth} ({type(exc).__name__}); splitting in half and retrying{budget_note}",
            file=sys.stderr,
        )
        mid = len(chunk) // 2
