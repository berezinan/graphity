...
def _build_server(graph_path: str):
    ...
    if _cfg["kind"] == "arcadedb":
        ...
        # Build the trigram query index eagerly so the first query doesn't pay
        # the one-time build (a few seconds on a large graph). serve exists to
        # answer queries, so absorbing the cost at startup beats spiking a random
        # first request; it's cached on G.graph, so queries just reuse it.
