...
def _score_record(
    ...
    if tiered and norm_terms:
        score += tiered * (matched / len(norm_terms)) ** 2
    return score
...
def _score_terms(terms: list[str], idf: dict[str, float]) -> tuple[list[str], str, float]:
    ...
    # query word must not double-count every tier, and with coverage scaling in
    # _score_record it would also inflate the matched-term ratio (#1602).
...
def _score_nodes(G: nx.Graph, terms: list[str]) -> list[tuple[float, str]]:
    ...
    # _score_terms centralizes the (norm_terms, joined, joined_w) triple so a DB
    # backend weights the whole-query tier exactly as this in-memory scan does.
...
def _build_server(graph_path: str):
    ...
    # Backend selection: JSON (default) loads graphs into the per-graph context
    # cache below and supports the G-dependent resources; ArcadeDB connects to
    # the running server (one database per project) and leaves G unset (those
    # resources degrade gracefully below).
    _IS_ARCADE = resolve_backend_config(graph_path)["kind"] == "arcadedb"

    # Per-graph context cache: resolved graph.json path -> {key, G, communities,
    # backend}. The server's default graph is just the first entry; a tool call
    # carrying a project_path adds its own. Routing every graph through one cache
    # means the eager trigram index and the mtime+size hot-reload behave
    # identically for the default graph and for any project graph.
    _default_graph_path = graph_path
    _ctx_lock = threading.Lock()
    _ctx_cache: dict[str, dict] = {}

    def _load_ctx(path: str):
        """Return (G, communities, backend) for a graph.json path, reusing a
        cached context until the file's (mtime, size) changes and then
        transparently rebuilding it. Unlike ``_load_graph`` it never exits the
        process on a missing/corrupt file — it raises, so a bad project_path
        surfaces as a tool error instead of killing a server that is happily
        serving other projects.

        ArcadeDB mode: the context is keyed by path alone (no graph.json file is
        needed — the database name derives from the path) and G stays None, so
        queries go to the DB backend and G-dependent extras degrade gracefully.
        """
        if _IS_ARCADE:
        ...
        with _ctx_lock:
            ent = _ctx_cache.get(path)
            if ent is not None and ent["key"] == key:
                return ent["G"], ent["communities"], ent["backend"]  # another thread built it
            try:
            ...
            be = JsonBackend(new_G, comm)
            _ctx_cache[path] = {"key": key, "G": new_G, "communities": comm, "backend": be}
            return new_G, comm, be
    ...
    try:
        G, communities, backend = _load_ctx(_default_graph_path)
        ...
        G, communities, backend = None, {}, None

    def _select_graph(project_path) -> None:
        nonlocal G, communities, backend, active_graph_path
        path = _resolve_graph_path(project_path)
        G, communities, backend = _load_ctx(path)
