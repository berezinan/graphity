...
def _bfs(G: nx.Graph, start_nodes: list[str], depth: int) -> tuple[set[str], list[tuple]]:
    ...
    # Completion needs the GRAPH, which the kernel deliberately does not take:
    # it runs over `neighbors`/`degree` callables so a DB-backed adjacency can
    # reuse it (query_backend). Upstream's #2323 call landed inside the kernel
    # in the merge, where `G` does not exist; it belongs here.
    visited, edges_seen = _bfs_core(G.neighbors, G.degree, start_nodes, depth, threshold)
...
def _dfs(G: nx.Graph, start_nodes: list[str], depth: int) -> tuple[set[str], list[tuple]]:
    ...
    # reuse it (query_backend). Upstream's #2323 call landed inside the kernel
    # in the merge, where `G` does not exist; it belongs here.
    visited, edges_seen = _dfs_core(G.neighbors, G.degree, start_nodes, depth, threshold)
    _complete_induced_edges(G, visited, edges_seen)
    return visited, edges_seen
...
def _build_server(graph_path: str):
    ...
    # Backend selection: JSON (default) loads graphs into the context cache below
    # and supports the G-dependent resources; ArcadeDB connects to the running
    # server (one database per project) and leaves G unset (those resources
    # degrade gracefully below).
    _IS_ARCADE = resolve_backend_config(graph_path)["kind"] == "arcadedb"
    _arcade_ctx: dict[str, dict] = {}
    ...
    def _load_ctx(path: str):
        ...
        # Kept out of _GraphContextCache: that cache evicts by graph.json
        # (mtime, size), which an ArcadeDB context has no file to provide.
        if _IS_ARCADE:
            ent = _arcade_ctx.get(path)
            if ent is None:
                with _arcade_lock:
                    ent = _arcade_ctx.get(path)
                    if ent is None:
                        be = require_backend(config=resolve_backend_config(path))
                        ent = {"G": None, "communities": {}, "backend": be}
                        _arcade_ctx[path] = ent
