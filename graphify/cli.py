...
def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        ...
        _result = render_query(
            ...
            # graph_path, не gp: `gp` связан только в JSON-ветке, а на пути
            # arcadedb его нет вовсе — тот же отказ, что чинил c147cde.
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from graphify.query_backend import (
            resolve_backend_config, require_backend, JsonBackend, render_path,
        )
        ...
        if _cfg["kind"] == "arcadedb":
            ...
            # Stash the stored arc order per edge BEFORE the graph is built:
            # under --undirected the loaded graph forgets which end was the
            # caller, and the renderer would print every segment forwards. Keep
            # markers already present (#2309) — on a canonicalized graph they,
            # not the arc order, carry the truth.
            _raw = dict(
                _raw,
                links=[
                    {
            ...
            # directed/multigraph как у апстрима: multigraph сохраняет
            # параллельные рёбра одной пары (#2074), иначе напечатанная
            # связь может оказаться не той, что несёт пройденная пара.
            _raw = {**_raw, "directed": not undirected, "multigraph": True}
            try:
                G = json_graph.node_link_graph(_raw, edges="links")
            except TypeError:
                G = json_graph.node_link_graph(_raw)
        ...
        if not result.found:
            ...
            # Through render_path, not a second copy of its wording: this branch
            # duplicated the renderer, so the directed/undirected distinction and
            # the "related" relation fallback reached one caller and not the other.
            print(render_path(_dc_replace(result, warnings=[]),
                              source_label, target_label))
            sys.exit(0)
        for _w in result.warnings:
            print(_w, file=sys.stderr)
        # Warnings already went to stderr above; strip them so the renderer does
        # not repeat them on stdout.
        print(render_path(_dc_replace(result, warnings=[]),
                          source_label, target_label))
        querylog.log_query(
            kind="path",
        ...
        try:
            ...
            _overlay = _llo(Path(graph_path).resolve())
            _entry = _overlay.get(str(result.detail.id))
            if _entry:
